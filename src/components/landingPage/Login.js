...
export default function Login(props) {
  ...
  const handlePatientLogin = async (healthID,emailID, password) => {

    if(emailID === "" && healthID === ""){
      props.settoastCondition({
        status: "error",
        message: "Please Enter Email or HealthID",
      });
    ...
    if (data.errors) {
      ...
      setEmailError(data.errors.emailID);
  ...
  const handleLogin = async (e) => {
    ...
    switch (Toggle) {
      case "Patient":
        handlePatientLogin(username,email, password);
        break;
      case "Doctor":
  ...
  return (
    <div className="bg-sky-100 flex flex-col justify-items-center items-center py-4 px-4 rounded shadow-md lg:w-3/4 w-full my-7 ml-auto ">
      ...
      <div className="flex bg-bgsecondary w-fit justify-between rounded">
        <button
          className={
            Toggle === "Patient"
              ? "py-1 px-6 text-lg font-poppins font-bold cursor-pointer rounded bg-slate-100"
      ...
      <form className="flex flex-col w-full px-8" onSubmit={handleLogin}>
        ...
        />
        {Toggle === "Patient" && <>
        <h1 className="font-poppins pt-2 pb-1 text-lg font-bold">OR</h1>
        <label
          htmlFor="email"
          className="font-poppins pt-2 pb-1 text-lg font-bold"
        >
        ...
        {Loading ? (
          ...
          <button
            type="submit"
            className="text-lg mt-10 bg-slate-300 py-1 px-3 rounded font-semibold font-poppins shadow-sm hover:bg-slate-100"
          >
            Login
          </button>
        )}
